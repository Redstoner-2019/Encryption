...
public class Encryption {
    ...
    public static void main(String[] args) throws Exception {
        KeyPair skeyPair = generateRSAKeyPair();
        PublicKey spuk = skeyPair.getPublic();
        PrivateKey sprk = skeyPair.getPrivate();

        KeyPair ckeyPair = generateRSAKeyPair();
        PublicKey cpuk = ckeyPair.getPublic();
        PrivateKey cprk = ckeyPair.getPrivate();

        String message = "Hello, World!";
        message = encrypt(message, sprk);

        System.out.println(message);

        message = decrypt(message, spuk);
